...
import { jsPDF } from 'jspdf' // cambio: import nombrado
...
export interface ConfirmEventData {
  ...
  user?: string
  evidencias?: File[] // NUEVO: archivos adjuntos para previsualizar
}
...
export function ConfirmEventDialog({ open, onOpenChange, data, onConfirm }: ConfirmEventDialogProps) {
  const { serviceId, type, status, message, coords, user, evidencias } = data
  ...
  // Crear URLs temporales para previsualizar imágenes y limpiarlas
  ...
  // Ref del contenedor a convertir en PDF (versión “print” con layout amplio)
      ...
      const canvas = await html2canvas(node, {
        ...
        onclone: (doc) => {
          ...
          if (el) {
            ...
            // Sobrescribe SIEMPRE estilos problemáticos en todos los descendientes
            el.querySelectorAll<HTMLElement>('*').forEach((child) => {
              ...
              // Evita variables CSS que pudieran expandirse a oklch()
              child.style.removeProperty('--*' as any)
            })
          }
        },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription id="confirm-desc">Vista previa del evento antes de guardar o descargar.</DialogDescription>
        </DialogHeader>
              ...
              <div className="flex flex-wrap items-start gap-2">
                {previews.map((p) => (
                  <img
                    ...
                    src={p.url}
                    alt={p.name}
                    className="max-h-56 max-w-full rounded border object-contain"
                  />
                ))}
              </div>
            </div>
        ...
        {/* Contenido “print” para PDF: ancho mayor y evidencias grandes */}
        <div
          id="pdf-print" // <-- agrega un id para localizar en el clon
          ref={pdfRef}
          style={{
            // Mantener renderizado pero no visible
            position: 'fixed',
            inset: 0,
            opacity: 0,          // en vez de visibility:hidden
            pointerEvents: 'none',
            width: 900,          // layout ancho para buena calidad
            margin: '0 auto',
            background: 'white',
            padding: 24,
            color: '#111',
